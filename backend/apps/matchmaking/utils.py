...
def nearby_requests(
    lat: float, lon: float, radius_km: float, *, game=None, exclude_user=None, queryset: Optional[Iterable[MatchmakingRequest]] = None
...
def join_or_create_party(user, game):
    ...
    with transaction.atomic():
        parties = list(GameParty.objects.select_for_update().filter(game=game, status=GameParty.STATUS_OPEN).order_by("created_at"))
        ...
        MatchmakingRequest.objects.filter(user=user, game=game, status=MatchmakingRequest.STATUS_PENDING).update(
            status=MatchmakingRequest.STATUS_MATCHED
        )
