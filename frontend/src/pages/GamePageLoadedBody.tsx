...
export function GamePageLoadedBody({
  ...
  const isTrunc = !logic.translating && fullText.length > DLIMIT;
  const dispText =
    isTrunc && !logic.descExpanded ? fullText.slice(0, DLIMIT) + '…' : fullText;
  ...
  return (
    <>
              ...
              <Tooltip title="Coup de cœur" arrow>
                <Box
                  ...
                  sx={{
                    ...
                    borderRadius: '12px',
                    cursor: 'pointer',
                    background: logic.userGame?.is_favorite
                      ? 'rgba(255,61,61,0.12)'
                      : 'rgba(0,0,0,0.03)',
                    border: logic.userGame?.is_favorite
                      ? '1px solid rgba(255,61,61,0.35)'
                      : '1px solid rgba(0,0,0,0.06)',
                    color: logic.userGame?.is_favorite ? accent : muted,
                ...
                >
                  ...
                  <Typography
                    sx={{
                      fontFamily: GAME_PAGE_FONT,
                      fontSize: 12.5,
                      fontWeight: logic.userGame?.is_favorite ? 700 : 500,
                      color: 'inherit',
                    }}
                  >
                    Favori
      ...
      <Box sx={{ ...card(noHov), p: { xs: '20px', md: '26px 30px' } }}>
        <ReviewSection
          gameId={logic.djangoId ? String(logic.djangoId) : ''}
